...
export default async function TrekPage({ params }: Props) {
    ...
    return (
      <main style={{ minHeight: "100vh", background: "#f5eedd", color: "#2b241c" }}>
        <section style={{ maxWidth: "672px", margin: "0 auto", padding: "64px 24px" }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "1.8rem", fontStyle: "italic", marginBottom: "16px" }}>Trek not found</h1>
          <Link href="/" style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "#3e5169" }}>← back to the map</Link>
        </section>
      </main>
    );
  }
  ...
  const parentState = allStates.find((s) =>
    (s as { trekNames?: string[] }).trekNames?.includes(trek.name) ||
    s.name === trek.state
  );
  ...
  return (
    <main className="page-fade-in" style={{ minHeight: "100vh", background: "#ede4d0", color: "#2b241c" }}>
      ...
      {/* Content */}
      <section style={{ maxWidth: "800px", margin: "-24px auto 0", padding: "0 32px 80px", position: "relative", zIndex: 1 }}>
        ...
        <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.68rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "#6b5f4f", marginBottom: "10px" }}>
          {trek.state}
          {parentState && <Link href={`/states/${parentState.id}`} style={{ color: "#3e5169", marginLeft: "6px", textDecoration: "none" }}>↗</Link>}
        </p>
        ...
        {/* ── Memory corner ── */}
        <div style={{ borderRadius: "12px", padding: "24px 28px", marginBottom: "12px", background: "rgba(180,196,210,0.14)", border: "1px solid rgba(62,81,105,0.2)" }}>
        ...
        <EditTrekContent
          trekId={trek.id}
          initialYourLines={trek.yourLines}
          initialPrompts={trek.prompts}
        />

        {/* Gallery divider */}
        <div style={{ display: "flex", alignItems: "center", gap: "14px", margin: "32px 0" }}>
        ...
        <div style={{ marginTop: "56px", paddingTop: "22px", borderTop: "1px solid rgba(43,36,28,0.1)", display: "flex", gap: "28px", alignItems: "center" }}>
          {parentState && <Link href={`/states/${parentState.id}`} style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "#3e5169", textDecoration: "none" }}>← {parentState.name}</Link>}
